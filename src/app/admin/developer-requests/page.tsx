...
const PI_SDK_SCRIPT_ID = 'escrowpi-admin-requests-pi-sdk';
...
export default function AdminDeveloperRequestsIndex() {
  ...
  const refreshDeveloperRequests = useCallback(
  ...
  const handleRefreshRequests = useCallback(() => {
    refreshDeveloperRequests(requestFilterMode);
  }, [refreshDeveloperRequests, requestFilterMode]);
  ...
  const toggleRequestFilterMode = useCallback(() => {
    ...
    refreshDeveloperRequests(nextMode);
  }, [requestFilterMode, refreshDeveloperRequests]);
  ...
  }, [piToken, requestFilterMode, refreshDeveloperRequests]);
  ...
  const renderRequestCard = useCallback(
    (request: DeveloperRequestRecord, index: number) => {
      ...
      const formEntries = Object.entries(request.formData ?? {}).filter(([fieldKey]) => fieldKey !== 'callbackUrls');
      ...
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-6 py-6">
        <header className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-3xl font-semibold text-gray-900 flex-1">All developer requests</h1>
            <Link
              href="/admin"
          ...
          <p className="text-sm text-gray-600">Browse every onboarding or rotation request, update statuses, and leave notes.</p>
        </header>
        ...
        <article className="rounded-2xl border p-5 shadow-sm" style={ADMIN_SURFACE_STYLE}>
          ...
          <p className="mt-2 text-sm text-gray-600">Expand any request to see Pi IDs, form submissions, and admin notes.</p>
          ...
          {!requestsError && requests.length === 0 && !requestsLoading && (
            <p className="mt-4 text-sm text-gray-500">No developer requests yet.</p>
          )}
          {requestsLoading && (
